...
void Sendbyte7219(char add_1, char dat_1, char add_2, char dat_2) {
  unsigned char i, j;
...
void T0_isr(void) __interrupt (1) {
    ...
    accel = (int)(((int)Single_ReadI2C(208, 0x3D) << 8) | Single_ReadI2C(208, 0x3E));
    ud = 0;
    if (accel > 5000) {
        ud = 1;
        m_1[9] = m_1[19] = 255;
    ...
    if (cnt % 10 == 0) {
        ...
        for (i = 1; i < 9; i++) Sendbyte7219(i, m[i], i, m[i + 10]);
        ...
        for (i = 1; i < 19; i++) {
            if (ud == 1) { 
                ...
                for (j = 128; (m[i] & j) > 0; j >>= 1); 
                if (j != 0) m[i] = (m[i] << 1) | (128 - j) << 1; 
            }
        }
        if (cnt % 30 != 0 && cnt % 50 != 0) Shift();
    }
...
void main(void) {
    // Init MAX7219
    Sendbyte7219(0x0C, 0x01, 0x0C, 0x01);
    Sendbyte7219(0x0F, 0x00, 0x0F, 0x00);
    Sendbyte7219(0x09, 0x00, 0x09, 0x00);
    Sendbyte7219(0x0B, 0x07, 0x0B, 0x07);
    Sendbyte7219(0x0A, 0x00, 0x0A, 0x00);
...
//  Single_WriteI2C(208, 0x1B, 8);
//  Single_WriteI2C(208, 0x1C, 0);
//  Single_WriteI2C(208, 0x1D, 3);
//  Single_WriteI2C(208, 0x37, 34);
//  Single_WriteI2C(208, 0x38, 0);

	TMOD = 0x01;						// Set Timer 1 to  mode 0 & Timer 0 mode 1. (16-bit timer)
	TH0  = (65536 - 50000) / 256;		// Load initial higher 8 bits into Timer 0
	TL0  = (65536 - 50000) % 256;		// Load initial lower 8 bits into Timer 0
